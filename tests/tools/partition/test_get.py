...
from osdu_wireline.tools.partition.get import partition_get

PARTITION_URL = "https://test.osdu.com/api/partition/v1/partitions/osdu"
...
@pytest.mark.asyncio
async def test_partition_get_success(osdu_env):
    """Test successful partition retrieval."""
    with aioresponses() as mocked:
        mocked.get(PARTITION_URL, payload=MIXED_PROPERTIES)
...
@pytest.mark.asyncio
async def test_partition_get_with_redacted_sensitive(osdu_env):
...
@pytest.mark.asyncio
async def test_partition_get_not_found(osdu_env):
...
@pytest.mark.asyncio
async def test_partition_get_with_sensitive_values(osdu_env):
...
@pytest.mark.asyncio
async def test_partition_get_exclude_sensitive(osdu_env):
